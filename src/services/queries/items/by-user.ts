...
export const itemsByUser = async (userId: string, opts: QueryOpts) => {
  ...
  const { total, documents } = await client.ft.search(getItemsIndexKey(), query, {
    ON: 'HASH',
    SORTBY: sortCriteria,
    LIMIT: {
  ...
  return {
    ...
    items: documents.map(document => deserialize(document.id.replace(getItemsKey(''), ''), document.value as any))
  };
};
